...
class CovidModel(tf.keras.Model):
    ...
    def _parse_inputs(self, inputs):
        ...
        (r_t,
         warmup_asymp,
         warmup_mild,
         warmup_extreme) = inputs

        r_t = tf.squeeze(r_t)

        warmup_asymp = {}
        warmup_asymp['everyone'] = tf.squeeze(warmup_asymp)

        warmup_mild = {}
        warmup_mild['everyone'] = tf.squeeze(warmup_mild)
        
        warmup_extreme = {}
        warmup_extreme['everyone'] = tf.squeeze(warmup_extreme)
